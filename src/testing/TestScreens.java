	...
	public void keyPressed(KeyEvent arg0) {
		...
		case ExperimentControl.EXP_FEEDBACK:
			...
			long trial_time_in_ms = Globals.GLOBAL_TIMER.toc();
                        ...
                        LogfileRow lr = new LogfileRow(trialNum, trial_time_in_ms, myConverge.getBase(), myConverge.getTrialParam(), myConverge.getDifference(), correctKey==currentKey, !isDescending, isBaseOnLeft?LogfileRow.RIGHT:LogfileRow.LEFT, SCALE, DOT_SIZE, NUM_POINTS_LEFT, DOT_STYLE);
			...
			this.trialNum ++;
			if(myConverge.isConverged()) expCtrl.setExpEnd();
			break;
		...
		case ExperimentControl.EXP_DISPLAY:
			...
			if(isBaseOnLeft){
				// base on left
				currentScreen = new TestTwoCorrScreen(myConverge.getBase(), myConverge.getTrialParam(), NUM_POINTS_LEFT, ERR_LEFT, SCALE, DOT_SIZE, DOT_STYLE);
				currentScreen.setCorrectKey(KEY_R);
			}else{
				// base on right
				currentScreen = new TestTwoCorrScreen( myConverge.getTrialParam(), myConverge.getBase(), NUM_POINTS_RIGHT, ERR_RIGHT, SCALE, DOT_SIZE, DOT_STYLE);
