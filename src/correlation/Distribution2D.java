...
public class Distribution2D implements XMLWriteable{
	...
	public static void sort(Comparable[] input) {
   	...
   	public boolean nextReliableCorrelatedUniforms(double r, int size, double error) {
		if(size < 0 || error < 0) return false;
		data.clear();
	...
	public boolean turnIntoTransformedCorrelatedGaussian(double r, int size, double error){
		if(!turnIntoCorrelatedGaussian(r, size, error)) return false;
...
//~*~*~*~*~*~*~*~*  UNCOMMENT THIS LINE FOR THE DOUBLE Y CONDITION*~*~*~*~*~*~*~*~*
	...
	public Point2D.Double getMinCoords(){
		...
		while(itr.hasNext()){
			...
			currValX = tmp.x;
			currValY = tmp.y;
			if(currValX < minValX) minValX = currValX;
			if(currValY < minValY) minValY = currValY;
		}
	...
	public Point2D.Double getMaxCoords(){
		...
		while(itr.hasNext()){
			...
			currValX = tmp.x;
			currValY = tmp.y;
			if(currValX > maxValX) maxValX = currValX;
			if(currValY > maxValY) maxValY = currValY;
		}
  ...
  // a method to fill the list of styles
	...
	public Image getImage(int width, int height, int numPoints, double scaling, double dpointSize, int iPointStyle, int idotHue){
		...
		// Axis
		if(isDrawAxis) DrawUtil.drawAxis(g2, graphOriginX, width, height-graphOriginY, height - height, Color.BLACK);
		...
		g2.setColor(Color.BLACK);
		g2.setFont(myFon);
		if(isDrawLabels) g2.drawString(xlabel, width/2-Util.getStringWidth(xlabel, myFon)/2, height - graphOriginY/2 + 1);

		// Text - vertical
		g2.rotate(Math.PI/2.0);
		if(isDrawLabels) g2.drawString(ylabel, height/2-Util.getStringWidth(ylabel, myFon)/2, -(graphOriginX/2 + 1));
